...
import {
  softDeleteFieldDailySectionNoteReply,
  updateFieldDailySectionNoteReply,
...
async function authorize(projectId: string, reportDateParam: string | undefined) {
...
export async function PATCH(
  req: NextRequest,
  context: { params: Promise<{ id: string; noteId: string; replyId: string }> },
) {
  const { id: projectId, noteId, replyId } = await context.params;
  const parsed = PatchReplySchema.safeParse(await req.json());
  if (!parsed.success) return NextResponse.json({ error: "Invalid input" }, { status: 400 });

  const auth = await authorize(projectId, parsed.data.reportDate);
  if ("error" in auth && auth.error) return auth.error;

  const result = await updateFieldDailySectionNoteReply({
  ...
  return NextResponse.json({ reply: result, reportDate: auth.reportDate });
}
...
export async function DELETE(
  ...
  const result = await softDeleteFieldDailySectionNoteReply({
    ownerUserIds: auth.ownerUserIds!,
    projectId,
    reportDate: auth.reportDate!,
    noteId,
