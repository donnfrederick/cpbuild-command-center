...
type Params = { id: string; obsId: string; cid: string };

export async function PATCH(req: NextRequest, { params }: { params: Promise<Params> }) {
  const session = await getSession();
  if (!session?.user) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });

  const { id: projectId, obsId, cid } = await params;
  ...
  const comment = await db.observationComment.findFirst({
    where: { id: cid, observationId: obsId, deletedAt: null },
    include: { observation: { select: { projectId: true } } },
  });

  if (!comment || comment.observation.projectId !== projectId) {
  ...
  const updated = await db.observationComment.update({
    where: { id: cid },
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<Params> }) {
  ...
  const comment = await db.observationComment.findFirst({
    where: { id: cid, observationId: obsId },
    include: { observation: { select: { projectId: true } } },
  });
  ...
  await db.observationComment.update({ where: { id: cid }, data: { deletedAt: new Date() } });
