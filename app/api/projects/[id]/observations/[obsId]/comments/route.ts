...
type Params = { id: string; obsId: string };
...
export async function GET(req: NextRequest, { params }: { params: Promise<Params> }) {
  ...
  const observation = await db.projectObservation.findUnique({
    where: { id: obsId, projectId },
    select: { id: true },
  });
  if (!observation) return NextResponse.json({ error: "Observation not found" }, { status: 404 });

  const comments = await db.observationComment.findMany({
    where: { observationId: obsId, deletedAt: null },
    include: {
...
export async function POST(req: NextRequest, { params }: { params: Promise<Params> }) {
  ...
  try {
    const comment = await db.$transaction(async (tx) => {
      const created = await tx.observationComment.create({
        data: {
          observationId: obsId,
    ...
    void (async () => {
      try {
        ...
        if (!mentionRl.ok) {
          logMentionEmailActorThrottled("observation_comment", {
            actorUserId: resolvedUserId,
            denied: mentionRl,
            projectId,
            observationId: obsId,
        ...
        for (const u of mentionedUsers) {
          void sendMentionEmail({
            ...
            projectUrl: `${APP_URL}/en/projects/${projectId}?openObservation=${obsId}`,
          }).catch((err) => console.warn("[mention-email]", err));
        }
      } catch (err) {
        console.warn("[mention-notify obs comment]", err);
      }
    })();

    return NextResponse.json(comment, { status: 201 });
  } catch (err) {
    console.error("[obs comments POST]", err);
