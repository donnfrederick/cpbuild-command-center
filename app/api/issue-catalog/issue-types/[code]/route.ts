...
export async function PATCH(req: NextRequest, { params }: RouteParams) {
  ...
  try {
    const row = await db.issueTypeCatalog.update({
      where: { code },
      data: {
        ...(parsed.data.displayName !== undefined
          ? { displayName: parsed.data.displayName.trim() }
          : {}),
        ...(parsed.data.requiresVisual !== undefined
          ? { requiresVisual: parsed.data.requiresVisual }
          : {}),
    ...
    console.error("[issue-catalog/issue-types PATCH]", err);
    return NextResponse.json({ error: "Failed to update issue type" }, { status: 500 });
  }
}
