...
export async function PATCH(req: NextRequest, { params }: RouteParams) {
  ...
  try {
    const row = await db.responsiblePartyCatalog.update({
      where: { code },
      data: {
    ...
    console.error("[issue-catalog/responsible-parties PATCH]", err);
    return NextResponse.json({ error: "Failed to update responsible party" }, { status: 500 });
  }
}
