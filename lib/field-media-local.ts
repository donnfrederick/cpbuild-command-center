...
const ALLOWED_FIELD_MEDIA_FOLDERS = new Set([
  "issues",
  "observations",
  "comments",
  "issue-comments",
...
/** Read local field-media only when file size is within maxBytes (avoids loading oversized files). */
export async function readLocalFieldMediaFileWithinMaxBytes(
  storageKey: string,
  maxBytes: number,
): Promise<Buffer | null> {
  const abs = resolveSafeAbsolutePath(storageKey);
...
export async function unlinkLocalFieldMediaKeys(keys: string[]): Promise<void> {
  for (const key of keys) {
    ...
    try {
      ...
      const err = e as NodeJS.ErrnoException;
      if (err.code !== "ENOENT") {
        console.error("[field-media-local] unlink failed:", err);
      }
    }
  }
}
...
/**
 * Public origin for storageUrl (localhost, ngrok, or Railway).
 * Prefer request headers; fall back to NEXTAUTH_URL / APP_BASE_URL / AUTH_URL; then localhost:3002.
 */
