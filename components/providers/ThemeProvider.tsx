...
/**
 * Script-free theme provider (replaces `next-themes` ThemeProvider).
 * `next-themes` always renders an inline `<script>` for flash prevention; React 19 logs that as an error
 * when the script is created during client render. We sync from localStorage in `useLayoutEffect` before
 * paint instead — same tradeoff as deferring the old provider, without a `<script>` node in the tree.
 */
...
export function ThemeProvider({
  ...
  /** Bumps when `prefers-color-scheme` changes (only used while `enableSystem`). */
