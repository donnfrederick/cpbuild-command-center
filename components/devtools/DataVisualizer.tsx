...
const FK_MAP: Record<string, Record<string, string>> = {
  User:                  { roleId: "Role" },
  Invite:                { roleId: "Role", sentById: "User" },
  RolePermission:        { roleId: "Role", permissionId: "Permission" },
  UserSpecialPermission: { userId: "User", grantedById: "User" },
  OfflinePreference:     { userId: "User" },
  DesignTokenSnapshot:   { savedById: "User" },
  ProjectRow: {
    projectId:      "Project",
    scopeTypeId:    "ScopeType",
    locationTypeId: "LocationType",
    costTypeId:     "CostType",
    installTeamId:  "InstallTeam",
    uomTypeId:      "UomType",
  },
  FeedbackReport: { userId: "User" },
};
  ...
  const fetchTableData = useCallback(async () => {
    ...
    try {
      ...
      if (validFilters.length > 0) {
        params.set("filters", JSON.stringify(validFilters.map(({ id: _id, ...f }) => f)));
      }
